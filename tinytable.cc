...
cmd_delrow(struct TinyTable * const tt, vector<string> &params)
{
  ...
  if (params.size() < 1) {
    cerr << "[delrow] need a row name" << endl;
    return;
  }
  for (vector<string>::iterator it = params.begin(); it != params.end(); it++ ){
    for(vector<string>::iterator ic = tt->columns.begin(); ic != tt->columns.end(); ic++ ){
...
cmd_delelem(struct TinyTable * const tt, vector<string> &params)
{
  ...
  if (tt->columns.size() == 0) {
    cerr << "[delelem] Table not defined." << endl;
    return;
  }
  if (params.size() != 2) {
    cerr << "[delelem] Need exactly 3 parameters" << endl;
    return;
  }
  string key = combine_key(params[0], params[1]);
  lldb_del(tt, key);
...
save_table_scheme(struct TinyTable * const tt)
{
  ...
  // 1. check if table scheme exists
  
  // 2. use lldb_set to store key: num, val: columns[n] to lldb
  int i = 0;
  string key;
  for(vector<string>::iterator ic = tt->columns.begin(); ic != tt->columns.end(); ic++ ){
    key = std::to_string(i);
